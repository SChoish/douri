#!/usr/bin/env bash
# AntMaze residual × subgoal grid @ 600 epochs.
#
# Order (IDM top-1 baselines first, then top-2):
#   large l1 → medium m1 → giant g1 → large l2 → medium m2 → giant g2
# Each baseline: 4 cells (rd_sd, rd_sa, ra_sd, ra_sa) = 24 runs total.
#
# Per cell: skip if train_epochs finished; resume from latest checkpoint if partial.
#
# Usage:
...
#
# Optional env: PYTHON_BIN, RUNS_ROOT, OUTDIR, LOGDIR, TRAIN_EPOCHS, DOURI_ROOT,
#   SWEEP_QUEUE="top1_only"  (default: top1 then top2)
#   BASELINE_FILTER="l1"   (run one baseline only)
#   CELL_FILTER="rd_sd"      (run one 4-way cell only: rd_sd|rd_sa|ra_sd|ra_sa)
...
OUTDIR="${OUTDIR:-$REPO_ROOT/scripts/sweep_generated/antmaze_res_subgoal_grid_600ep}"
LOGDIR="${LOGDIR:-$REPO_ROOT/scripts/sweep_logs/antmaze_res_subgoal_grid_600ep}"
...
echo "[$(date -Is)] sweep start repo=$REPO_ROOT epochs=$TRAIN_EPOCHS queue=$SWEEP_QUEUE" | tee "$MASTER_LOG"

_run_group_for () {
  local scale="$1" baseline="$2" res_short="$3" sg_short="$4"
  echo "sweep600_res_sg_${scale}_${baseline}_${res_short}_${sg_short}"
}
...
_run_cell () {
  ...
  local yname="antmaze_${scale}_${baseline}_${res_short}_${sg_short}.yaml"
  ...
  echo "=== $(date -Is) CELL $scale $baseline ${res_short}_${sg_short} (group=$run_group) ===" | tee -a "$MASTER_LOG"
  ...
  echo "=== $(date -Is) DONE $scale $baseline ${res_short}_${sg_short} ===" | tee -a "$MASTER_LOG"
}
...
echo "[$(date -Is)] sweep queue finished. Master: $MASTER_LOG"
