"""GOUB-inspired Phase-1 **path-supervised** training (YAML + timestamped runs).

Same layout as ``main_goub_phase1.py``, but defaults to
:class:`utils.datasets.PathHGCDataset` (adds ``trajectory_segment``) and
:class:`agents.goub_phase1_path.GOUBPhase1PathAgent`.

Baseline endpoint-only training remains ``main_goub_phase1.py``.
"""
...
from agents.goub_phase1_path import GOUBPhase1PathAgent
...
def _default_yaml_path():
    return os.path.join(_impl_dir(), 'config', 'goub_phase1_path_antmaze.yaml')
...
config_flags.DEFINE_config_file('agent', 'agents/goub_phase1_path.py', lock_config=False)
...
def _setup_file_logger(run_dir: str) -> logging.Logger:
    ...
    logger = logging.getLogger('goub_phase1_path')
...
def _make_gc_dataset(ds_plain, config):
    name = str(config.get('dataset_class', 'PathHGCDataset'))
    if name == 'PathHGCDataset':
        return PathHGCDataset(ds_plain, config)
    raise ValueError(
        f'Unsupported dataset_class {name!r} for main_goub_phase1_path '
        '(path training requires PathHGCDataset).'
    )
...
def main(_):
    ...
    agent_tok = _sanitize_token(str(config.get('agent_name', 'goub_phase1_path')))
    ...
    train_dataset = _make_gc_dataset(train_ds_plain, config)
    val_dataset = _make_gc_dataset(val_ds_plain, config) if val_ds_plain is not None else None
    ...
    if seg_horizon != goub_horizon:
        raise ValueError(
            f'Path Phase-1 requires trajectory_segment horizon ({seg_horizon}) == goub_N ({goub_horizon}).'
        )
    run_logger.info(
    ...
    agent = GOUBPhase1PathAgent.create(FLAGS.seed, example_batch['observations'], config, ex_actions=ex_act)
