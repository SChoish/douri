#!/usr/bin/env bash
# Puzzle 4x4 grid sweep: alpha=0 only, gap>0, train 200ep then resume to 600 if metric improves.
#
#   CUDA_VISIBLE_DEVICES=0 nohup bash scripts/with_jax_cuda.sh \
#     bash scripts/sweep_alpha0_4x4_to_600.sh >> alpha0_4x4_sweep600.master.log 2>&1 &
...
CSV_OUT="${ROOT}/sweep_results/puzzle_fbr_displacement_grid_alpha0_4x4_600.csv"

TS="$(date +%Y%m%d_%H%M%S)"
LAUNCH_LOG_ROOT="${ROOT}/launch_logs/alpha0_4x4_sweep600/${TS}"
...
mapfile -t CONFIGS < <(printf '%s\n' "${ROOT}/config/grid_fbr_displacement_puzzle/puzzle_4x4_a0p0_"*.yaml | sort)
# Exclude gap=0 (puzzle_4x4_a0p0_gap0p0_*)
filtered=()
for cfg in "${CONFIGS[@]}"; do
  base="$(basename "${cfg}")"
  [[ "${base}" == *"_gap0p0_"* ]] && continue
  filtered+=("${cfg}")
done
CONFIGS=("${filtered[@]}")
NCFG="${#CONFIGS[@]}"
echo "[4x4-a0-600] ${NCFG} configs (alpha=0, gap>0)"
...
for cfg in "${CONFIGS[@]}"; do
  ...
  echo "================================================================================"
  echo "[4x4-a0-600] (${cfg_idx}/${NCFG}) ${cfg}"
  ...
  set -e
  if [[ "${ec1}" -ne 0 ]]; then
    echo "[4x4-a0-600] stage1 FAILED exit=${ec1}"
...
echo "[4x4-a0-600] done. CSV=${CSV_OUT}"
