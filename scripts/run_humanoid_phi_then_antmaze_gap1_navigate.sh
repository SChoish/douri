#!/usr/bin/env bash
# Order: humanoidmaze medium -> large (phi + subgoal_value_gap_scale=5.0), then
# antmaze medium / large / giant navigate (subgoal_value_gap_scale=1.0 yaml).
# Sequential GPU runs (nohup + wait per stage inside this script).
#
# Usage:
#   cd /home/choi/douri
#   export CUDA_VISIBLE_DEVICES=0   # optional
#   nohup bash scripts/run_humanoid_phi_then_antmaze_gap1_navigate.sh \
#     > nohup_logs/seq_humanoid_gap1_antmaze_$(date +%Y%m%d_%H%M%S).log 2>&1 &
#
